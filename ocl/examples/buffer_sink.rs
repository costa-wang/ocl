...
fn buffer_sink() -> ocl::Result<()> {
    ...
    print_type_name(&writer_0);
    // let mut write_guard = block_on(async {
    //         return writer_0.await;
    // });

    // let buffer_sink: BufferSink<_> = WriteGuard::release(write_guard).into();

    print_type_name(&buffer_sink);
    ...
    //         print_type_name(&buffer_sink);
    //         block_on(async {
    //             let buffer_sink = buffer_sink.flush().enq().unwrap();
    //             return buffer_sink.await;
    //         });
    //     })?);
